...
GuiLayoutConfig() {
    cfg := {
        ...
        titleW: 100,    ; 寬度
        ; Control: 控制項 (右側)
        controlX: 130,  ; X座標，右側的 開始位置
        radioW: 55,     ; 單選按鈕寬度(含文字)
    ...
    ; getRowY用，按照順序 排序 每一行 ROW
    cfg.rowContentList := ["adminMode", "zenkakuBlock", "triggerSide", "triggerKey", "pauseKey"]
...
InitGUI(*) {
    ...
    getRowY(rowName) { ; 根據 行名 計算Y座標

        for index, name in cfg.rowContentList {
            if (name = rowName) {
                return cfg.firstRowY + (cfg.rowGap * (index - 1))
            }
        }

        return cfg.firstRowY
    }
    OffsetDDLY(rowName) { ; 根據 行名 調整 DDL Y座標
        return GetRowY(rowName) + cfg.ddlYOffset ; Drop Down List 預設位置太高，這個函式用來調整 Y座標
    }
    ...
    ; Zenkaku Block
    zenkakuBlockTitle := mainGui.AddText(
        "x" cfg.marginX
        " y" getRowY("zenkakuBlock")
        " w" cfg.titleW
        " h" cfg.rowH
        " +0x200", "Zenkaku Block:"
    )
    zenkakuBlockOn := mainGui.AddRadio("x" GetRadioX(0) " y" getRowY("zenkakuBlock") " w" cfg.radioW " h" cfg.rowH,
    "ON")
    zenkakuBlockOff := mainGui.AddRadio("x" GetRadioX(1) " y" getRowY("zenkakuBlock") " w" cfg.radioW " h" cfg.rowH,
    "OFF")
    AddLabelTip(zenkakuBlockTitle, "禁用全形: 請先切為半形狀態 `n阻止 Shift + Space 切換成全形，改輸出一般空白") ; label 提示
    ...
    lastRowBottomY := getRowY(cfg.rowContentList[cfg.rowContentList.Length]) + cfg.rowH ; 最後一行的底部 Y座標
...
SaveHandler(guiForm, mainGui) {
    ...
    ; 空白 錯誤檢查
    if (adminMode = "" || zenkakuBlock = "" || triggerSide = "" || triggerKey = "" || pauseKey = "") {
    ...
    IniWrite(zenkakuBlock, SettingsFile, "Hotkeys", "ZenkakuBlock")
...
InitAltNumMap() { ; 設定主鍵 (PrimaryKey) & 對應的輸出(OutputKey) (主鍵 := 要送出的按鍵)
    ...
    ; 主要拿來擋 放大鏡功能
    g_AltNumMap["="] := "="
    ; 綁定自訂 FN 物件：透過切換語系 API，解決有注音底線時，標點符號始終卡在最後的問題
    g_AltNumMap[","] := SendTextByLanguageSwitch.Bind(",")
}
...
InitApp() {
    ...
    OnMessage(0x200, WM_MOUSEMOVE) ; 顯示提示
...
; 對 label 新增 提示
AddLabelTip(ctrl, text) {
    global labelToolTipMap
    labelToolTipMap[ctrl.Hwnd] := text
}
...
WM_MOUSEMOVE(wParam, lParam, msg, hwnd) {
    ...
    if (labelToolTipMap.Has(ctrlHwnd)) {
        ToolTip(labelToolTipMap[ctrlHwnd])
    }
    if (ctrlHwnd = "") {
        ToolTip()
    }
...
SendMappedKey(outputKey, *) {
    ; 映射目標為 FN 物件 (如標點符號 API 處理): 直接執行該 FN
    if (HasMethod(outputKey, "Call")) {
        outputKey.Call()
        return
    }
...
; 利用系統訊息切換語系佈局，強制使輸入法斷字並安全送出字元（不模擬鍵盤）
SendTextByLanguageSwitch(charToSend) {
    ; 獲取當前焦點視窗
    hwndActive := ControlGetFocus("A")
    if (!hwndActive)
        hwndActive := WinExist("A")

    if (!hwndActive) {
        SendInput("{Text}" . charToSend)
        return
    }

    ; 1. 獲取當前線程的輸入法佈局 (HKL)
    ; 0x0404 通常是繁體中文，0x0409 是美式英文
    currentHKL := DllCall("GetKeyboardLayout", "UInt", DllCall("GetWindowThreadProcessId", "Ptr", hwndActive, "Ptr", 0),
    "Ptr")

    ; 2. 載入並啟用純英文（美式鍵盤）佈局
    ; 0x04090409 = 英文(美國)
    englishHKL := DllCall("LoadKeyboardLayout", "Str", "00000409", "UInt", 1, "Ptr")

    ; 3. 透過 API 發送切換輸入法請求 (WM_INPUTLANGCHANGEREQUEST = 0x0050)
    ; 這個動作會強迫微軟注音在被卸載前，交出並固定（斷字）所有底線字
    SendMessage(0x0050, 1, englishHKL, , "ahk_id " . hwndActive)

    ; 稍微給系統一隻微小的喘息時間（保證切換完成）
    Sleep(10)
    ...
    ; 5. 瞬間還原回原本的輸入法佈局
    SendMessage(0x0050, 1, currentHKL, , "ahk_id " . hwndActive)
}
